...
Your answer: yes, that means all relevant data has been included in the forecast
...
Your answer: only if ???????? ??? 0 for all ????,
and ???? has a natural zero.

> d. If your model doesn't forecast well, you should make it more complicated.

Your answer: no, maybe its too complicated and should rather be simplified or another model is simply better for that data

> e. Always choose the model with the best forecast accuracy as measured on the test set.

Your answer: no, the test set might not represent the reality
...
Your answer: almost normally distributed but not uncorrelated for the first lags
...
mean1 <- meanf(dowjones, h=5)
mean2 <- meanf(dowjones, h=10) 
mean3 <- meanf(dowjones, h=15)

autoplot(dowjones) +
  autolayer(naive3, PI=FALSE, series="naive 5") +
  autolayer(naive2, PI=FALSE, series="naive 10") +
  autolayer(naive1, PI=FALSE, series="naive 15") +
  xlab("Time") + ylab("Closing Price (US$)") +
  ggtitle("Dow Jones index") +
  guides(colour=guide_legend(title="Forecast"))

autoplot(dowjones) +
  autolayer(mean3, PI=FALSE, series="meanf 5") +
  autolayer(mean2, PI=FALSE, series="meanf 10") +
  autolayer(mean1, PI=FALSE, series="meanf 15") +
  xlab("Time") + ylab("Closing Price (US$)") +
  ggtitle("Dow Jones index") +
  guides(colour=guide_legend(title="Forecast"))
...
```{r}
# todo
ibm.train <- window(ibmclose, end = c(300))
ibm.test <- window(ibmclose, start = c(301))
```
...
```{r}
# todo
sales.train <- window(hsales, end = 1995)
sales.test <- window(hsales, start = 1995)
```
...
# todo
h <- length(sales.test)
m.f <- meanf(sales.train, h=h)
 
sn.f <- snaive(sales.train, h=h)
rwd.f <- rwf(sales.train, drift=TRUE, h=h)

autoplot(hsales) +
  xlab("Day") +
  ggtitle("sales of new one-family houses in the USA") +
  autolayer(m.f$mean, col=2, series="Mean method") +
  autolayer(sn.f$mean, col=3, series="SNaive method") +
  autolayer(rwd.f$mean, col=4, series="Drift method")

 accuracy(m.f,sales.test)
 accuracy(sn.f,sales.test)
 accuracy(rwd.f,sales.test)
```
